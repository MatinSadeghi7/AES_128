...
public class ramz2 {
    ...
    private static String[][] fix = {{"0e", "0b", "0d", "09"},
                                     {"09", "0e", "0b", "0d"},
                                    {"0d", "09", "0e", "0b"},
                                    {"0b", "0d", "09", "0e"}};
    ...
    public static void main(String[] args) {
        ...
        String cipherin = sc.next();

        int size = 16;
        String[] keyarr = new String[size];
        String[] cipherarr = new String[size];
        ...
        for (int i = 0; i < 32; i = i + 2) {
            cipherarr[i / 2] = cipherin.substring(i, i + 1).concat(cipherin.substring(i + 1, i + 2));
        }
        String[][] ciphertext = new String[4][4];
        int z = 0;
        for (int i = 0; i < 4; i++) {
            for (int j = 0; j < 4; j++) {
                ciphertext[i][j] = cipherarr[z];
                z++;
            }
        }
        ...
        for (int i = 0; i < 44; i = i + 4) {
            for (int j = 0; j < 7; j = j + 2) {
                ...
                subkeys[i / 4] = (temp[4 * q].concat(temp[4 * q + 1]).concat(temp[4 * q + 2]).concat(temp[4 * q + 3]));
            }
            q++;
        }


        ciphertext = addRoundKeys(subkeys[10], ciphertext);
        shiftrow(ciphertext);
        ciphertext = subbyte(ciphertext, 4);
        for (int i = 0; i < 9; i++) {
            ciphertext = addRoundKeys(subkeys[9 - i], ciphertext);
            String [][] cipher2 = new String[4][4];
            for (int j = 0; j <4 ; j++) {
                for (int k = 0; k <4 ; k++) {
                    cipher2[j][k] = ciphertext[k][j];
        ...
        ciphertext = addRoundKeys(subkeys[0], ciphertext);
    ...
    static String[][] subbyte(String[][] ciphertext, int a) {

        //subbyte
        for (int i = 0; i < a; i++) {
            for (int j = 0; j < a; j++) {
                int first = Integer.parseInt(ciphertext[i][j].substring(0, 1), 16);
                int second = Integer.parseInt(ciphertext[i][j].substring(1, 2), 16);
                if (bool == false) {
                    ciphertext[i][j] = Integer.toHexString(SBOX[first][second]);
                } else {
                    ciphertext[i][j] = Integer.toHexString(INV_SBOX[first][second]);
                }
                if (ciphertext[i][j].length() == 1) {
                    ciphertext[i][j] = "0".concat(ciphertext[i][j]);
                }
            }
        }
        return (ciphertext);
    ...
    static String[][] addRoundKeys(String sub, String[][] ciphertext) {
        ...
        for (int i = 0; i < 4; i++) {
            for (int j = 0; j < 4; j++) {
                subkey[i][j] = myxor(subkey[i][j], ciphertext[i][j]);
            }
    ...
    static void mixcolumn(String[][] ciphertext) {
        int[][] out = new int[4][4];
        for (int i = 0; i < 4; i++) {
            for (int j = 0; j < 4; j++) {
                out[i][j] = 0;
                for (int k = 0; k < 4; k++) {
                    out[i][j] ^= mult(Integer.parseInt(fix[i][k] , 16) , Integer.parseInt(ciphertext[k][j], 16));
                }
            }
        }
        for (int i = 0; i < 4; i++) {
            for (int j = 0; j < 4; j++) {
                ciphertext[i][j] = Integer.toHexString(out[i][j]);

                if (Integer.toHexString(out[i][j]).length() == 1) {
                    ciphertext[i][j] = "0".concat(Integer.toHexString(out[i][j]));
                }
                if (Integer.toHexString(out[i][j]).length() == 3) {
                    ciphertext[i][j] = Integer.toHexString(out[i][j]).substring(1, 3);
                }
